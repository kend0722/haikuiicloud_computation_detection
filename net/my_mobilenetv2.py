...
def _make_divisible(v, divisor, min_value=None):
    """
    :param v: 输入通道数与通道倍数的乘积.
    :param divisor: 除数, 值为 8.
    :param min_value:
    :return: 离 v 最近的 8 的倍数.
    """
...
class MyMobileNetV2(nn.Module):
    ...
    def __init__(self,kpt_num=3,pretrained=True):
        super(MyMobileNetV2, self).__init__()
        self.backbone = MobileNetV2() #pretrained:预先训练的，预训练参数，等于True,说明采用预训练参数
        if pretrained == True: #加载预训练参数
            pretrained_dict  = torch.load(r"./mobilenet_v2-b0353104.pth")
